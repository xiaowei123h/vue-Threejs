<template>
    <div class="webglMaterialsNodes-container">
...
export default {
    data() {
        return {
			...
			serialized: false,
			textures: {
				brick: { url: 'static/textures/brick_diffuse.jpg' },
				grass: { url: 'static/textures/terrain/grasslight-big.jpg' },
				grassNormal: { url: 'static/textures/terrain/grasslight-big-nm.jpg' },
				decalDiffuse: { url: 'static/textures/decal/decal-diffuse.png' },
				decalNormal: { url: 'static/textures/decal/decal-normal.jpg' },
				cloud: { url: 'static/textures/lava/cloud.png' },
				spherical: { url: 'static/textures/envmap.png' }
			},
            param: null,
            premTexture: null,
            pmremCube: null,
    ...
    mounted() {
        ...
        this.param = { example: new URL(window.location.href).searchParams.get('e') || 'mesh-standard' }
        var that = this
        this.cubemap = function () {
            ...
            var textureCube = new that.$THREE.CubeTextureLoader().load(urls, that.updatePREM)
            that.library[ textureCube.uuid ] = textureCube
            return textureCube
        }()
    ...
    methods: {
        getTexture(name) {
            ...
            if (! texture) {
                texture = textures[ name ].texture = new this.$THREE.TextureLoader().load(textures[ name ].url)
        ...
        updatePREM(textureCube) {
            ...
            var minFilter = this.pmremCube.minFilter
            var magFilter = this.pmremCube.magFilter
            var generateMipmaps = this.pmremCube.generateMipmaps
        ...
        init() {
            ...
            this.renderer.uuid = this.$THREE.MathUtils.generateUUID() // generate to library
            ...
            var light
            this.lightGroup.add(new this.$THREE.AmbientLight(0x464646))
            light = new this.$THREE.DirectionalLight(0xffddcc, 1)
            light.position.set(1, 0.75, 0.5)
            this.lightGroup.add(light)
            light = new this.$THREE.DirectionalLight(0xccccff, 1)
            light.position.set(- 1, 0.75, - 0.5)
            this.lightGroup.add(light)
            this.teapot = new TeapotBufferGeometry(15, 18)
            this.mesh = new this.$THREE.Mesh(this.teapot)
            this.scene.add(this.mesh)
            this.library[ this.renderer.uuid ] = this.renderer
            this.library[ this.camera.uuid ] = this.camera
            this.library[ this.mesh.uuid ] = this.mesh
            this.updatePREM()
        ...
        updateMaterial() {
            ...
            if (this.rtTexture) {
                delete library[ this.rtTexture.texture.uuid ]
            ...
            var name = this.param.example,
                defaultSide = this.$THREE.DoubleSide,
                mtl
            this.clearGui()
            switch (name) {
                case 'phong':
                    ...
                    var mask = new Nodes.SwitchNode(new Nodes.TextureNode(this.getTexture("decalDiffuse")), 'w')
                    mtl.color = new Nodes.TextureNode(this.getTexture("grass"))
                    ...
                    mtl.normal = new Nodes.NormalMapNode(new Nodes.TextureNode(this.getTexture("grassNormal")))
                ...
                case 'standard':
                    ...
                    // GUI
                    this.addGui('color', mtl.color.value.getHex(), (val) => {
                        mtl.color.value.setHex(val)
                    }, true)
                    this.addGui('roughnessA', roughnessA.value, (val) => {
                        roughnessA.value = val
                    }, false, 0, 1)
                    this.addGui('metalnessA', metalnessA.value, (val) => {
                        metalnessA.value = val
                    }, false, 0, 1)
                    this.addGui('roughnessB', roughnessB.value, (val) => {
                        roughnessB.value = val
                    }, false, 0, 1)
                    this.addGui('metalnessB', metalnessB.value, (val) => {
                        metalnessB.value = val
                    }, false, 0, 1)
                    this.addGui('normalScale', normalScale.value, (val) => {
                ...
                case 'prem':
                    ...
                    this.addGui('intensity', intensity.value, (val) => {
                        intensity.value = val
                    }, false, 0, 2)
                    this.addGui('roughnessA', roughnessA.value, (val) => {
                        roughnessA.value = val
                ...
                case 'sub-slot':
                    ...
                    // GUI
                    this.addGui('radiance', radiance.value, (val) => {
                        radiance.value = val
                    }, false, 0, 2)
                    this.addGui('irradiance', irradiance.value, (val) => {
                        irradiance.value = val
                    }, false, 0, 2)
                    this.addGui('roughness', roughness.value, (val) => {
                        roughness.value = val
                    }, false, 0, 1)
                    this.addGui('metalness', metalness.value, (val) => {
                        metalness.value = val
                    }, false, 0, 1)
                    this.addGui('normalScale', normalScale.value, (val) => {
                        normalScale.value = val
                ...
                case 'mesh-standard':
                    ...
                    function updateMaterial() {
                        ...
                        mtl = useNodeMaterial ? new Nodes.MeshStandardNodeMaterial() : new this.$THREE.MeshStandardMaterial()
                        // default syntax (backward-compatible)
                        mtl.map = useMap ? this.getTexture("brick") : undefined
                        mtl.normalMap = useNormals ? this.getTexture("decalNormal") : undefined
                        mtl.normalScale = oldMaterial ? oldMaterial.normalScale : new this.$THREE.Vector2(.5, .5)
                        mtl.envMap = cubemap
                    ...
                    this.updateMaterial()
                    // GUI
                    this.addGui('use node material', useNodeMaterial, (val) => {
                        useNodeMaterial = val
                        this.updateMaterial()
                    })
                    this.addGui('roughness', mtl.roughness, (val) => {
                        mtl.roughness = val
                    }, false, 0, 1)
                    this.addGui('metalness', mtl.roughness, (val) => {
                        mtl.metalness = val
                    }, false, 0, 1)
                    this.addGui('normalX', mtl.normalScale.x, (val) => {
                        mtl.normalScale.x = val
                    }, false, - 1, 1)
                    this.addGui('normalY', mtl.normalScale.y, (val) => {
                        mtl.normalScale.y = val
                    }, false, - 1, 1)
                    this.addGui('sat. (node)', sataturation.value, (val) => {
                        sataturation.value = val
                    }, false, 0, 2)
                    this.addGui('colors', useMap, (val) => {
                        useMap = val
                        this.updateMaterial()
                    }, false)
                    this.addGui('normals', useNormals, (val) => {
                        useNormals = val
                        this.updateMaterial()
                    }, false)
                    this.addGui('side', {
                        DoubleSided: this.$THREE.DoubleSide,
                        FrontSided: this.$THREE.FrontSide,
                        BackSided: this.$THREE.BackSide
                    }, (val) => {
                ...
                case 'physical':
                    ...
                    mtl.clearcoatNormal = new Nodes.NormalMapNode(new Nodes.TextureNode(this.getTexture("grassNormal")));
                    ...
                    this.addGui('reflectivity', reflectivity.value, (val) => {
                        reflectivity.value = val;
                    }, false, 0, 1);
                    this.addGui('clearcoat', clearcoat.value, (val) => {
                        clearcoat.value = val;
                    }, false, 0, 1);
                    this.addGui('clearcoatRoughness', clearcoatRoughness.value, (val) => {
                        clearcoatRoughness.value = val;
                    }, false, 0, 1);
                    this.addGui('clearcoatNormalScale', clearcoatNormalScale.value, (val) => {
                ...
                case 'wave':
                    ...
                    mtl.position = blend
                    // GUI
                    this.addGui('speed', speed.value, (val) => {
                        speed.value = val
                    }, false, 0, 10)
                    this.addGui('scale', scale.value, (val) => {
                        scale.value = val
                    }, false, 0, 3)
                    this.addGui('worldScale', worldScale.value, (val) => {
                        worldScale.value = val
                    }, false, 0, 1)
                    this.addGui('colorA', colorA.value.getHex(), (val) => {
                        colorA.value.setHex(val)
                    }, true)
                    this.addGui('colorB', colorB.value.getHex(), (val) => {
                        colorB.value.setHex(val)
                    }, true)
                    this.addGui('useNormals', false, (val) => {
                ...
                case 'rim':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    defaultSide = this.$THREE.FrontSide
                    var intensity = 1.3
                    ...
                    this.addGui('intensity', intensity, (val) => {
                        intensity = val
                        viewZ.b.z = - intensity
                        rim.b.value = intensity
                    }, false, 0, 3)
                    this.addGui('power', power.value, (val) => {
                        power.value = val
                    }, false, 0, 6)
                    this.addGui('xray', false, (val) => {
                        if (val) {
                            mtl.emissive = color
                            mtl.alpha = rimPower
                            mtl.blending = this.$THREE.AdditiveBlending
                            mtl.depthWrite = false
                        } else {
                            mtl.emissive = rimColor
                            mtl.alpha = null
                            mtl.blending = this.$THREE.NormalBlending
                            mtl.depthWrite = true
                        }
                        mtl.needsUpdate = true
                    })
                ...
                case 'color-adjustment':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    var texture = new Nodes.TextureNode(this.getTexture("brick"))
                ...
                case 'uv-transform':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    var translate = new this.$THREE.Vector2()
                    var rotate = 0
                    var scale = new this.$THREE.Vector2(1, 1)
                    var texture = new Nodes.TextureNode(this.getTexture("brick"))
                    ...
                    function updateUVTransform() {
                        texture.uv.setUvTransform(translate.x, translate.y, scale.x, scale.y, this.$THREE.MathUtils.degToRad(rotate))
                    }
                    this.addGui('translateX', translate.x, (val) => {
                        translate.x = val
                        updateUVTransform()
                    }, false, 0, 10)
                    this.addGui('translateY', translate.y, (val) => {
                        translate.y = val
                        updateUVTransform()
                    }, false, 0, 10)
                    this.addGui('scaleX', scale.x, (val) => {
                        scale.x = val
                        updateUVTransform()
                    }, false, .1, 5)
                    this.addGui('scaleY', scale.y, (val) => {
                        scale.y = val
                        updateUVTransform()
                    }, false, .1, 5)
                    this.addGui('rotate', rotate, (val) => {
                        rotate = val
                        updateUVTransform()
                    }, false, 0, 360)
                    break
                case 'bump':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    var diffuse = new Nodes.TextureNode(this.getTexture("brick"))
                    var bumpMap = new Nodes.BumpMapNode(new Nodes.TextureNode(this.getTexture("brick")))
                    ...
                    // GUI
                    this.addGui('scale', bumpMap.scale.value, (val) => {
                        bumpMap.scale.value = val
                    }, false, - 2, 2)
                    this.addGui('color', true, (val) => {
                ...
                case 'blur':
                    ...
                    mtl = new Nodes.PhongNodeMaterial()
                    var diffuse = new Nodes.TextureNode(this.getTexture("brick"))
                    var blur = new Nodes.BlurNode(new Nodes.TextureNode(this.getTexture("brick")))
                    mtl.color = blur
                    // GUI
                    this.addGui('radiusX', blur.radius.x, (val) => {
                        blur.radius.x = val
                    }, false, 0, 15)
                    this.addGui('radiusY', blur.radius.y, (val) => {
                        blur.radius.y = val
                    }, false, 0, 15)
                    break
                case 'spherical-reflection':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    mtl.environment = new Nodes.TextureNode(this.getTexture("spherical"), new Nodes.ReflectNode(Nodes.ReflectNode.SPHERE))
                    break
                case 'fresnel':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    ...
                    // GUI
                    this.addGui('reflectance', reflectance.value, (val) => {
                ...
                case 'layers':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    var tex1 = new Nodes.TextureNode(this.getTexture("grass"))
                    var tex2 = new Nodes.TextureNode(this.getTexture("brick"))
                    ...
                    var mask = new Nodes.TextureNode(this.getTexture("decalDiffuse"), uvScale)
                    ...
                    mtl.color = blend
                    // GUI
                    this.addGui('offset', offset.value, (val) => {
                        offset.value = val
                    }, false, 0, 1)
                    this.addGui('scale', scale.value, (val) => {
                        scale.value = val
                ...
                case 'saturation':
                    // MATERIAL
                    mtl = new Nodes.StandardNodeMaterial()
                    var tex = new Nodes.TextureNode(this.getTexture("brick"))
                ...
                case 'top-bottom':
                    // MATERIAL
                    mtl = new Nodes.PhongNodeMaterial()
                    var top = new Nodes.TextureNode(this.getTexture("grass"))
                    var bottom = new Nodes.TextureNode(this.getTexture("brick"))
                    ...
                    mtl.color = blend
                    // GUI
                    this.addGui('hard', hard.value, (val) => {
                        hard.value = val
                    }, false, 0, 20)
                    this.addGui('offset', offset.value, (val) => {
                        offset.value = val
                ...
                case 'dissolve':
                    ...
                    // GUI
                    this.addGui('threshold', threshold.value, (val) => {
                        threshold.value = val
                    }, false, - .3, 1.3)
                    this.addGui('borderSize', borderSize.value, (val) => {
                        borderSize.value = val
                    }, false, 0, .5)
                    this.addGui('color', color.value.getHex(), (val) => {
                        color.value.setHex(val)
                    }, true)
                    this.addGui('borderColor', borderColor.value.getHex(), (val) => {
                ...
                case 'dissolve-fire':
                    ...
                    this.addGui('fireSize', fireSize.value, (val) => {
                        fireSize.value = val
                    }, false, 0, .5)
                    this.addGui('burnedSize', burnedSize.value, (val) => {
                        burnedSize.value = val
                    }, false, 0, 1)
                    this.addGui('color', color.value.getHex(), (val) => {
                        color.value.setHex(val)
                    }, true)
                    this.addGui('fireStartColor', fireStartColor.value.getHex(), (val) => {
                        fireStartColor.value.setHex(val)
                    }, true)
                    this.addGui('fireEndColor', fireEndColor.value.getHex(), (val) => {
                        fireEndColor.value.setHex(val)
                    }, true)
                    this.addGui('burnedColor', burnedColor.value.getHex(), (val) => {
                        burnedColor.value.setHex(val)
                    }, true)
                    this.addGui('timeScale', timer.scale, (val) => {
                        timer.scale = val
                    }, false, 0, 2)
                    break
                case 'smoke':
                    ...
                    var cloudA = new Nodes.TextureNode(this.getTexture("cloud"), uvOffsetA)
                    var cloudB = new Nodes.TextureNode(this.getTexture("cloud"), uvOffsetB)
                    ...
                    mtl.alpha = clouds
                    // GUI
                    this.addGui('color', mtl.environment.value.getHex(), (val) => {
                ...
                case 'camera-depth':
                    ...
                    mtl.color = colors
                    // GUI
                    this.addGui('near', depth.near.value, (val) => {
                        depth.near.value = val
                    }, false, 1, 1200)
                    this.addGui('far', depth.far.value, (val) => {
                        depth.far.value = val
                    }, false, 1, 1200)
                    this.addGui('nearColor', colorA.value.getHex(), (val) => {
                        colorA.value.setHex(val)
                    }, true)
                    this.addGui('farColor', colorB.value.getHex(), (val) => {
                        colorB.value.setHex(val)
                    }, true)
                    break
                case 'caustic':
                    ...
                    // GUI
                    this.addGui('timeScale', timeScale.value, (val) => {
                        timeScale.value = val
                    }, false, 0, 5)
                    this.addGui('intensity', intensity.value, (val) => {
                        intensity.value = val
                ...
                case 'soft-body':
                    ...
                    // GUI
                    this.addGui('spring', velocity.params.spring, (val) => {
                        velocity.params.spring = val
                    }, false, 0, .95)
                    this.addGui('damping', velocity.params.damping, (val) => {
                        velocity.params.damping = val
                    }, false, 0, .95)
                    this.addGui('scale', scale.value, (val) => {
                        scale.value = val
                    }, false, 0, 3)
                    this.addGui('softBody', colorA.value.getHex(), (val) => {
                        colorA.value.setHex(val)
                    }, true)
                    this.addGui('rigidBody', colorB.value.getHex(), (val) => {
                        colorB.value.setHex(val)
                    }, true)
                    break
                case 'plush':
                    ...
                    this.addGui('mildness', mildness.value, (val) => {
                        mildness.value = val
                    }, false, 1, 2)
                    this.addGui('fur', fur.value, (val) => {
                        fur.value = val
                    }, false, 0, 2)
                    break
                case 'skin':
                case 'skin-phong':
                    ...
                    // GUI
                    this.addGui('skinColor', skinColor.value.getHex(), (val) => {
                        skinColor.value.setHex(val)
                    }, true)
                    this.addGui('bloodColor', bloodColor.value.getHex(), (val) => {
                        bloodColor.value.setHex(val)
                    }, true)
                    this.addGui('wrapLight', wrapLight.value, (val) => {
                        wrapLight.value = val
                    }, false, 0, 3)
                    this.addGui('wrapShadow', wrapShadow.value, (val) => {
                ...
                case 'toon':
                    ...
                    this.addGui('lineColor', lineColor.value.getHex(), (val) => {
                        lineColor.value.setHex(val)
                    }, true)
                    this.addGui('count', count.value, (val) => {
                        count.value = val
                    }, false, 1, 8)
                    this.addGui('lineSize', lineSize.value, (val) => {
                        lineSize.value = val
                    }, false, 0, 1)
                    this.addGui('lineInner', lineInner.value, (val) => {
                        lineInner.value = val
                    }, false, 0, 1)
                    this.addGui('ignoreIndirectLight', false, (val) => {
                ...
                case 'expression':
                    ...
                    Nodes.NodeLib.addKeyword('myCustomUv', () => {
                        return new Nodes.ReflectNode()
                    })
                    // GUI
                    addGui('speed', speed.value, (val) => {
                        speed.value = val
                    }, false, 0, 1)
                    break
                ...
                case 'bias':
                    ...
                    this.addGui('bias', bias.value, (val) => {
                        bias.value = val
                    }, false, 0, 1)
                    break
                case 'node-position':
                ...
                case 'node-reflect':
                    ...
                    var standardMaterial = new this.$THREE.MeshStandardMaterial({
                    ...
                    mtl = nodeMaterial
                    // GUI
                    this.addGui('node', true, (val) => {
                        mtl = val ? nodeMaterial : standardMaterial
                        mesh.material = mtl
                    })
                    this.addGui('roughness', nodeMaterial.roughness.value, (val) => {
                ...
                case 'void-function':
                    ...
                    position.keywords[ "tex" ] = new Nodes.TextureNode(this.getTexture("brick"))
                ...
                case 'conditional':
                    ...
                    mtl.color = cond
                    // GUI
                    this.addGui('a', a.value, (val) => {
                        a.value = val
                    }, false, 0, 1)
                    this.addGui('b', b.value, (val) => {
                        b.value = val
                    }, false, 0, 1)
                    this.addGui('a condition b', {
                    ...
                    this.addGui('if color', ifNode.value.getHex(), (val) => {
                        ifNode.value.setHex(val)
                    }, true)
                    this.addGui('else color', elseNode.value.getHex(), (val) => {
                ...
                case 'rtt':
                    ...
                    mtl.normal = bumpMap
                    // GUI
                    this.addGui('bump', bumpMap.scale.value, (val) => {
                        bumpMap.scale.value = val
                    }, false, - .5, .5)
                    this.addGui('scale', 2, (val) => {
                        uvTransform.setUvTransform(0, 0, val, val, 0)
                    }, false, 0, 8)
                    this.addGui('ignoreColor', false, (val) => {
                ...
                case 'temporal-blur':
                    ...
                    mtl.color = rttSave
                    // GUI
                    this.addGui('click to reset', false, () => {
                ...
                case 'triangle-blur':
                    ...
                    var blurredTexture = new Nodes.FunctionCallNode(blurtexture, {
                        texture: new Nodes.TextureNode(this.getTexture("brick")),
                        delta: delta,
                        uv: new Nodes.UVNode()
                    })
                    var color = new Nodes.MathNode(
                        new Nodes.TextureNode(this.getTexture("brick")),
                        blurredTexture,
                        alpha,
                        Nodes.MathNode.MIX
                    )
                    ...
                    this.addGui('deltaX', delta.x, (val) => {
                        delta.x = val
                    }, false, 0, 1)
                    this.addGui('deltaY', delta.x, (val) => {
                        delta.y = val
                    }, false, 0, 1)
                    break;
                ...
                case 'translucent':
                    ...
                    rtTexture = new this.$THREE.WebGLRenderTarget(window.innerWidth, window.innerHeight, { minFilter: this.$THREE.LinearFilter, magFilter: this.$THREE.NearestFilter, format: this.$THREE.RGBFormat })
                    ...
                    distanceMtl.side = this.$THREE.BackSide
                    ...
                    // GUI
                    this.addGui('frontColor', frontColor.value.getHex(), (val) => {
                        frontColor.value.setHex(val)
                    }, true)
                    this.addGui('backColor', backColor.value.getHex(), (val) => {
                        backColor.value.setHex(val)
                    }, true)
                    this.addGui('area', sss.b.value, (val) => {
                        sss.b.value = val
                    }, false, 0, 1)
                    break
            }
        ...
        serialize() {
            var json = this.mesh.material.toJSON()
            // replace uuid to url (facilitates the load of textures using url otherside uuid) e.g:
            var cloud = getTexture("cloud")
            this.NodeMaterialLoaderUtils.replaceUUID(json, cloud, "cloud")
            this.library[ "cloud" ] = cloud
            // --
            var jsonStr = JSON.stringify(json)
            console.log(jsonStr)
            var loader = new NodeMaterialLoader(null, this.library),
        ...
        animate() {
            ...
            if (this.move) {
                ...
                this.mesh.position.z = Math.cos(time) * 10
                this.mesh.position.y = Math.sin(time) * 10
            } else {
                this.mesh.position.z = this.mesh.position.y = 0
            }
            //this.mesh.rotation.z += .01
            // update material animation and/or gpu calcs (pre-renderer)
            this.frame.setRenderer(this.renderer).update(delta)
            if (this.mesh.material instanceof Nodes.NodeMaterial) {
                this.frame.updateNode(this.mesh.material)
            }
            // render to texture for sss/translucent material only
            if (rtTexture) {
                this.scene.overrideMaterial = rtMaterial
